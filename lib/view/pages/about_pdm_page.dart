...
class _AboutPdmPageState extends State<AboutPdmPage> {
  ...
  void initState() {
    ...
    if (cached != null &&
        (cached.profile != null || cached.structure.isNotEmpty)) {
      setState(() {
  ...
  Future<void> _loadData() async {
    ...
    setState(() {
      ...
      if (!profileResult.success && !structureResult.success) {
        _error = profileResult.message.isNotEmpty
  ...
  Widget _buildContactSection(bool isDark, OrganizationProfileModel? profile) {
    ...
    final hasSocial =
        (sm?.instagram ?? sm?.facebook ?? sm?.youtube ?? sm?.twitter) != null;
  ...
  Widget _buildMapPreview(bool isDark, {String? address, String? title}) {
    const LatLng officeLocation = LatLng(
      -7.949650126353061,
      112.60868403249327,
    );
    final locationTitle = title?.isNotEmpty == true
        ? title!
        : 'Kantor PDM Kota Malang';
    final locationAddress = address?.trim().isNotEmpty == true
        ? address!
        : 'Jl. Gajayana No. 28, Malang';
              ...
              HtmlWidget(
                ...
                customStylesBuilder: (element) {
                  if (element.localName == 'ul' || element.localName == 'ol') {
                    return {'padding-inline-start': '1.4em', 'margin': '0'};
                  }
                  if (element.localName == 'li') {
                    return {'margin-bottom': '4px'};
                  }
                  return {'margin': '0', 'padding': '0', 'text-align': 'left'};
                },
              ),
            ],
          ),
  ...
  Widget _buildStrukturList(
    bool isDark,
    List<OrganizationStructureModel> structure,
  ) {
              ...
              children: [
                if (s.photo.startsWith('http'))
                  ...
                  const CircleAvatar(
                    radius: 28,
                    backgroundColor: AppStyle.primary,
                    child: Icon(RemixIcons.user_3_fill, color: Colors.white),
                  ),
                const SizedBox(height: 12),
                Text(
                  s.name,
      ...
      children: programs
          .map(
            (p) => Container(
              ...
              decoration: BoxDecoration(
                color: isDark ? AppStyle.cardDark : const Color(0xFFF1F4F9),
                borderRadius: BorderRadius.circular(12),
                border: isDark
              ...
              child: Text(
                ...
                style: TextStyle(
                  fontSize: 13,
                  fontWeight: FontWeight.w600,
                  color: isDark ? AppStyle.accent : Colors.black87,
                ),
              ),
            ),
          )
