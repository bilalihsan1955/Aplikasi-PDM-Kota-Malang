...
class _AccountPageState extends State<AccountPage> {
  ...
  void _discardEditingToCache() {
    setState(() {
      ...
      _nameController.text = _cachedUser?.name ?? '';
      _emailController.text = _cachedUser?.email ?? '';
      _phoneController.text = _cachedUser?.phone ?? '';
      _nbmController.text = _cachedUser?.nbm ?? '';
      _positionController.text = _cachedUser?.position ?? '';
    });
  }
                        ...
                        Padding(
                          ...
                          child: Column(
                            children: [
                              ...
                              const SizedBox(height: 32),
                              _buildEditButton(context, isDark, isSubmitting),
                              if (_isEditing) ...[
                                const SizedBox(height: 14),
                                _buildSaveButton(context, isSubmitting),
                              ],
                              if (!_isEditing && !_isLoadingUser) ...[
                                const SizedBox(height: 14),
                                _buildChangePasswordButton(context, isDark, isSubmitting),
                              ],
      ...
      child: Stack(
        children: [
          ...
          Positioned(
            ...
            child: GestureDetector(
              onTap: () async {
                final source = await showGlassImageSourceDialog(context: context);
  ...
  Widget _buildAccountForm(bool isDark) {
    final enabled = _isEditing && !_isLoadingUser;
    return Column(
      children: [
        _buildTextField(
          label: 'Nama Lengkap',
          ...
          enabled: enabled,
          hintText: 'Nama untuk tampilan profil di aplikasi (bukan nama perangkat)',
        ),
        ...
        _buildTextField(
          ...
          hintText: 'Alamat email akun — dipakai sistem untuk login',
        ),
        const SizedBox(height: 20),
        _buildTextField(
          label: 'Nomor Telepon',
          ...
          hintText: 'Nomor aktif, tanpa spasi — hanya angka menurut format sistem',
        ),
        const SizedBox(height: 20),
        _buildTextField(
          label: 'NBM',
          ...
          inputFormatters: [FilteringTextInputFormatter.digitsOnly],
          hintText: 'Nomor bukti keanggotaan organisasi — opsional, hanya angka',
        ),
        const SizedBox(height: 20),
        _buildReadOnlyField(
      ...
      children: [
        ...
        TextField(
          ...
          decoration: InputDecoration(
            prefixIcon: Icon(icon, color: AppStyle.formPrefixIconColor(isDark), size: 20),
            filled: true,
            fillColor: fill,
            hintText: hintText,
            hintStyle: TextStyle(
            ...
            disabledBorder: OutlineInputBorder(
              borderRadius: BorderRadius.circular(16),
              borderSide: BorderSide(color: isDark ? Colors.white.withOpacity(0.06) : Colors.grey.shade200, width: 1),
            ),
      ...
      children: [
        ...
        Container(
          ...
          child: Padding(
            ...
            child: Row(
              children: [
                Icon(icon, color: AppStyle.formPrefixIconColor(isDark), size: 20),
  ...
  Widget _buildEditButton(BuildContext context, bool isDark, bool isSubmitting) {
    final borderColor = isDark ? Colors.white.withOpacity(0.05) : const Color(0xFFF1F4F9);
    final textColor =
        _isEditing ? Colors.redAccent : AppStyle.accentOnSurface(isDark);
    final icon = _isEditing ? RemixIcons.close_circle_line : RemixIcons.edit_2_line;
    final title = _isEditing ? 'Batal Edit' : 'Edit Data';
    final borderRadius = BorderRadius.circular(20);

    return PressFeedback(
      onTap: (_isLoadingUser || isSubmitting)
          ? null
          : () async {
  ...
  Widget _buildSaveButton(BuildContext context, bool isSubmitting) {
    final enabled = _isEditing && !_isLoadingUser && !isSubmitting;
    return Container(
      width: double.infinity,
      decoration: BoxDecoration(
          ...
          onTap: !enabled
              ...
              : () async {
                  final ok = await showGlassConfirmDialog(
                    ...
                    message:
                        'Perubahan akan dikirim ke server dan disimpan di perangkat Anda.',
                    confirmText: 'Simpan',
                    cancelText: 'Batal',
                  ...
                  if (result.success && result.user != null) {
                    setState(() {
                      ...
                      _nbmController.text = result.user!.nbm ?? '';
                      _positionController.text = result.user!.position ?? '';
                      _pickedAvatarBytes = null;
                      _isEditing = false;
                    });
  ...
  Widget _buildChangePasswordButton(BuildContext context, bool isDark, bool isSubmitting) {
    final borderRadius = BorderRadius.circular(20);
    final borderColor = isDark ? Colors.white.withOpacity(0.05) : const Color(0xFFF1F4F9);

    return PressFeedback(
      onTap: isSubmitting ? null : () => context.push('/profile/change-password'),
      borderRadius: borderRadius,
      pressedOverlayColor: isDark
          ? Colors.white.withOpacity(0.04)
          : Colors.black.withOpacity(0.03),
      child: Container(
        ...
        child: Row(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            Icon(RemixIcons.lock_password_line, color: AppStyle.accentOnSurface(isDark)),
            const SizedBox(width: 8),
            Text(
              'Ganti kata sandi',
              style: TextStyle(
